...
export default class Terminal {
  ...
  connect() {
    ...
    this.channel = this.socket.channel(this.channelName(), {client: 'atom'})
  ...
  labForChannel() {
    if (this.labSlug) { return this.labSlug }

    return atom.config.get(`${name}.openOnHome`) ? 'home' : 'temporary'
  }

  channelName() {
    return `session:${this.username}:${this.labForChannel()}`
  }
