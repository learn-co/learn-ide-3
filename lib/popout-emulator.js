...
class PopoutEmulator {
  constructor() {
    ...
    this.popoutWindow = remote.getCurrentWindow()
  ...
  subscribe() {
    ...
    bus.on(`popout-emulator:write:${this.popoutWindow.id}`, (text) => {
  ...
  sendToTerminal(data) {
    bus.emit(`popout-emulator:data:${this.popoutWindow.id}`, data)
  }
