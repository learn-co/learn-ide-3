...
class TerminalView extends View {
  ...
  writeToEmulator(text) {
    ...
    if (this.hasPopoutEmulator()) {
      bus.emit(`popout-emulator:write:${this.popout.id}`, text);
    }
  }
  ...
  loadPopoutEmulator() {
    return new Promise((resolve) => {
      ...
      this.popout = new BrowserWindow({title: `Learn IDE | ${this.terminal.labForChannel()}`, show: false, autoHideMenuBar: true});
      this.popout.loadURL(`file://${popoutEmulatorFile}`);

      bus.on(`popout-emulator:data:${this.popout.id}`, (data) => {
